...
function getOutlookPollIntervalMs() {
  ...
  // Keep interval in a safe range: 1m to 30m.
  return Math.min(Math.max(Math.floor(configured), 60 * 1000), 30 * 60 * 1000);
}

function getOutlookSyncIntervalMs() {
  const configured = Number(import.meta.env.VITE_OUTLOOK_SYNC_MS);
  if (!Number.isFinite(configured)) {
    return 10 * 60 * 1000;
  }

  // Keep heavy provider-sync in a safe range: 2m to 60m.
  return Math.min(Math.max(Math.floor(configured), 2 * 60 * 1000), 60 * 60 * 1000);
}

const OUTLOOK_POLL_INTERVAL_MS = getOutlookPollIntervalMs();
const OUTLOOK_SYNC_INTERVAL_MS = getOutlookSyncIntervalMs();
const OUTLOOK_POLL_ENABLED = String(import.meta.env.VITE_OUTLOOK_POLL_ENABLED || "true").toLowerCase() === "true";
...
function OutlookPage() {
  ...
  const lastOutlookSyncRef = useRef(0);

  const fetchOutlook = useCallback(async ({ syncProvider = false, forceProviderSync = false } = {}) => {
    if (!user) {
      setLoading(false);
      return;
    }
    try {
      // Do heavy provider sync less often to avoid overwhelming request volume.
      const nowMs = Date.now();
      const canSyncProvider = forceProviderSync || (syncProvider && (nowMs - lastOutlookSyncRef.current >= OUTLOOK_SYNC_INTERVAL_MS));

      if (outlookConnected && canSyncProvider) {
        try {
          await api.get(`/outlook/fetch/${user.id}`);
          lastOutlookSyncRef.current = nowMs;
        } catch (syncError) {
  ...
  const runSearch = useCallback(async (queryText) => {
    ...
    if (!query) {
      await fetchOutlook({ syncProvider: false });
      return;
    }
  ...
  useEffect(() => {
    ...
    fetchOutlook({ syncProvider: true, forceProviderSync: true });
  }, [fetchOutlook, runSearch, searchParams]);

  useEffect(() => {
    if (!OUTLOOK_POLL_ENABLED || !user || !outlookConnected) {
      return;
    }

    let inFlight = false;

    const pollCachedOutlook = async () => {
    ...
    const syncProviderOutlook = async () => {
      if (document.hidden || inFlight) {
        return;
      }
      ...
      inFlight = true;
      try {
        await fetchOutlook({ syncProvider: true });
      } finally {
        inFlight = false;
      }
    };

    const pollIntervalId = window.setInterval(pollCachedOutlook, OUTLOOK_POLL_INTERVAL_MS);
    const syncIntervalId = window.setInterval(syncProviderOutlook, OUTLOOK_SYNC_INTERVAL_MS);
